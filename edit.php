...
// Get book data
$stmt = $mysqli->prepare("SELECT * FROM `My books List` WHERE Id=?");
...
move_uploaded_file($_FILES['image']['tmp_name'], $uploadPath);
       ...
       // $uploadPath = "uploads/" . basename($imageName);
        ...
        $update = $mysqli->prepare("
            ...
            SET Name=?, Description=?, `Release date`=?, Rate=?, Category=?, image=?
            WHERE Id=?
        ");
        ...
        $update = $mysqli->prepare("
            UPDATE `My books List` 
            SET Name=?, Description=?, `Release date`=?, Rate=?, Category=?
            WHERE Id=?
        ");
...
<div class="container">
    <div class="card">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <input type="date" name="Release_date" value="<?php echo $book['Release date']; ?>" required>
			...
			<img src="<?php echo $book['image']; ?>" alt="Book Image" width="200">
